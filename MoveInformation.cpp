...
MoveInformation::MoveInformation(Piece pieceType1, bool isCapture1, Piece capturedPiece1, bool isAmbiguous1, bool isEnpassant1, bool isPromotion1, Piece promotionPiece1) {}
...
char MoveInformation::getPieceLetter(Piece pieceType)
{ // CHATGPT'D
    switch (pieceType)
    {
    case pawn:
...
std::string MoveInformation::getSquareName(int square)
{ // CHATGPT'D
...
void MoveInformation::printMoveInfo()
{
    ...
    if (isEnpassant)
        std::cout << "Special: En Passant\n";
    if (isKingCastle)
        std::cout << "Special: Kingside Castle\n";
    if (isQueenCastle)
        std::cout << "Special: Queenside Castle\n";
    if (isCheck)
        std::cout << "Check: Yes\n";
    if (isCheckMate)
        std::cout << "Checkmate: Yes\n";
    if (isAmbiguous)
        std::cout << "Note: Move was ambiguous | " << fromFile << ", " << fromRank << ".\n";
