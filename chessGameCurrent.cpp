...
//INLCUDING THIS CODE TO CIRCUMVENT CIRCULAR DEPENDENCIES ISSUE

class Board; // forward declaration
enum Color;  // if needed

namespace MoveGenerator {
    uint64_t attackedByPawns(const Board&, Color);
    uint64_t possibleKnightMovesBitBoard(const Board&, Color);
    uint64_t possibleBishopMovesBitBoard(const Board&, Color);
    uint64_t possibleRookMovesBitBoard(const Board&, Color);
    uint64_t possibleQueenMovesBitBoard(const Board&, Color);
    uint64_t possibleKingMovesBitBoard(const Board&, Color);
    std::vector<MoveInformation> generateLegalMoves(const Board&, Color);
    // declare anything else you use in Board methods
}
...
namespace MoveGenerator{ //uses move parser (uses moveInformation)
    ...
    */
        ...
        // BITBOARDS!!!!
        uint64_t possiblePawnMovesBitBoard(const Board& boardOfInterest, Color colorOfInterest){     // pawn moves cant capture...
...
class Board{ // uses MoveGeneration
private:
    // White pieces
    uint64_t whitePawns;
...
public:
    ...
    bool isSquareInCheck(Square squareOfInterest, Color colorOfSquare) const {
        ...
        uint64_t opponentMoves = MoveGenerator::attackedByPawns(*this, opponentColor) | MoveGenerator::possibleKnightMovesBitBoard(*this, opponentColor) | MoveGenerator::possibleBishopMovesBitBoard(*this, opponentColor) | 
                                MoveGenerator::possibleRookMovesBitBoard(*this, opponentColor) | MoveGenerator::possibleQueenMovesBitBoard(*this, opponentColor) | MoveGenerator::possibleKingMovesBitBoard(*this,opponentColor);
    ...
    // CASTLING
    bool getCastlePrivelege(Color side) const{
        if (side == white){
            return whiteCastlePrivelege;
        }
        return blackCastlePrivelege;
    }
    bool canKingCastle(Color color) const{ //check for if any of the 4 squares of relevance are in check... done later
    ...
    bool canQueenCastle(Color color) const{ //ensures piece in right location + not in check
...
namespace MoveParser{ //uses MoveInformation objects
...
public:
    ...
    void updateNotationForMoveList(std::vector<MoveInformation> &legalMovesList){
            for (auto& moves: legalMovesList){ //update the strings of all moves after bad moves removed or info updated
                moves.chessNotation = MoveParser::getMoveString(moves);
            }
        }
    void printMoveList(std::vector<MoveInformation> moveList){
    for (int i = 0; i < moveList.size();i++){
        std::cout << MoveParser::getMoveString(moveList.at(i)) << ", ";
    ...
    void makeManualGameHalfTurn (Color turn, std::string chessNotation){ // identical to takeGameHalfTurn but move is predetermined
        ...
        moveBeingMade = MoveParser::createMoveFromString(getBoard(),turn,chessNotation);
        
        MoveInformation matchingMove = getMatchingMove(getBoard(), MoveGenerator::generateLegalMoves(getBoard(), turn) , moveBeingMade);
    ...
    bool isStaleMate(Board boardState, Color colorTakingTurn){     
        ...
        if (colorTakingTurn == white){
            possibleMoves = MoveGenerator::generateLegalMoves(boardState, white);
        }
        else{
            possibleMoves = MoveGenerator::generateLegalMoves(boardState,black);
        }
